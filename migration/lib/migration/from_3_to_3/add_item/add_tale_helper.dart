...
class AddTaleHelper extends _AddTaleHelper {
  ...
  List<String>? _getText() => [
    ...
    "Мама завжди починала її пошепки, наче це була велика таємниця. Вона розповідала, що десь у лісі росте особлива ялинка — темна, мов ніч, тиха, як снігова заметіль. І під цією ялинкою раз на сто років з’являється шишка, яка світиться зсередини. Але це не просто чарівна річ — це випробування. Шишка виконує бажання, але лише ті, що йдуть від щирого серця. Усі інші бажання стають випробуванням. Вони перевіряють серце: чи в ньому є місце для інших, а не тільки для себе.",
    ...
    "І кожного разу, засинаючи під мамині лагідні слова, Белла уявляла ту чарівну шишку і мріяла про неї, як про щось надзвичайне і добре.",
    "Минали роки. Белла виросла. Мама переселилася до іншого дупла на південному узліссі, а Белла сама влаштувала затишне житло в дуплі старого дуба. Вона навчилася готувати смачний чай, підтримувати лад у домі та навіть допомагати сусідам із ремонтом хаток. Але хоч би скільки зими пройшло, Белла ніколи не перестала вірити в ту чарівну шишку з маминої казки.",
    ...
    "Вона обережно поклала її в кошик і побігла додому. Серце билося, наче в ньому співала зграя пташок, але в душі зростала тривога.",
    ...
    "— Чарівна шишка — це не подарунок, а дзеркало. Вона відкриває не просто можливості, а серце того, хто її тримає. Кожне бажання показує, наскільки ти готова бачити світ не лише очима, а й душею.",
    "2",
    ...
    "Потім з’явилось лисеня на ім’я Міла.",
    "— Я хочу, щоб усі мене слухались! Щоб усі робили, як я скажу!",
    ...
    "Софія мовчки дивилась на неї, а потім сказала:",
    "— Те, що ми хочемо, не завжди те, що нам потрібно. Іноді ліс вчить нас чекати. Іноді — втрачати. Але саме це робить нас сильнішими.  Чари — це випробування. Навіть найщиріше бажання може обернутись випробуванням, якщо воно зростає не з мудрості, а з бажання зробити все добре одразу. Якщо ти справді хочеш зробити добро — запитай себе, для кого воно. І чи справді ти чуєш, що потрібно — не тільки серцем, а й розумом.",
...
abstract class _AddTaleHelper extends BaseAddHelper {
  _AddTaleHelper(From3to3 migration)
    : super(migration, addItemName: 'Tale', folderName: 'tales');
  ...
  Future<bool> validate({bool post = false}) async {
    try {
      ...
      if (post) {
        ...
        assert(imageFile.existsSync(), 'Image for the tale was not found');

        if (tale.tags.contains(TaleTag.audio)) {
          final audioFile = File('${path}audio.mp3');
          assert(audioFile.existsSync(), 'Image for the tale was not found');
        }
      }
      return true;
    } catch (e) {
  ...
  Future<void> add() async {
    final createDateDelta = const Duration(days: 1);
    ...
    tale = TaleDto(
      id: nextId,
      name: taleName,
      createDate: DateTime.now().add(createDateDelta),
      updateDate: null,
      summary: summary,
      tags: tags,
      text: text,
  ...
  TextContentDto? _textContentDto() {
    ...
    final readingTime = readingTimeHelper.getReadingTime(paragraphs: text);
